...
2. Start services: `docker compose up --build` (exposes port 8080). Set `DATABASE_URL` to your Postgres instance; if you use the legacy `postgres://` prefix it will be rewritten to `postgresql://` automatically. Override `DB_PORT` if 5432 is taken. Uploads persist to `/data/uploads` by default—Docker and Fly volumes should mount this path.
...
## Tests
Run `pytest` (requires dependencies from `requirements.txt`). To include the dockerized persistence check, run `RUN_DOCKER_INTEGRATION=1 pytest -m integration` with Docker available.
